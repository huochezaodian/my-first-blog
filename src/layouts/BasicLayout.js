...
import AudioAffix from '../components/AudioAffix';
...
export default class BasicLayout extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.menus = getNavData('BasicLayout').reduce((arr, current) => arr.concat(current.children), []);
    this.state = {
      breadcrumbNameMap: this.getBreadcrumbName(),
    };
  }
  getChildContext() {
  ...
  getBreadcrumbName(){
    const menuData = getNavData('BasicLayout').reduce((arr, current) => arr.concat(current.children), []);
    const breadcrumbNameMap = {};
    this.getNavDataDetail(menuData, breadcrumbNameMap);
    return { ...breadcrumbNameMap };
  }
  getNavDataDetail(children, data){
    children.forEach((item) => {
      data[item.path] = item.name;
      if(item.children){
        this.getNavDataDetail(item.children, data);
      }
    })
  }
  ...
  onMenuClick = ({ key }) => {
    if (key === 'logout') {
      ...
      this.props.dispatch(routerRedux.push('/people'));
    }else if (key === 'setting'){
  ...
  render() {
    const { currentUser, collapsed, loading, location } = this.props;
    ...
    const menu = (
      <Menu className={styles.menu} selectedKeys={[]} onClick={this.onMenuClick}>
        <Menu.Item key="people"><Icon type="user" />个人中心</Menu.Item>
        ...
        <Layout>
              ...
              {currentUser.name ? (
                <Dropdown overlay={menu}>
                  <span className={`${styles.action} ${styles.account}`}>
                    <Avatar size="small" className={styles.avatar} src={photo} />
                    {currentUser.name}
                  </span>
                </Dropdown>
          ...
          <Content style={{ margin: '24px 24px 0', height: '100%' }}>
            ...
            <Switch>
              ...
              <Redirect exact from="/" to="/note/javascript" />
            ...
            <GlobalFooter
              ...
              copyright={
                <div>
                  Copyright <Icon type="copyright" /> 2017 基于Ant Design Pro个人出品
                </div>
              }
            />
          </Content>
